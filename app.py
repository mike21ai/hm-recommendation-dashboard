...
st.markdown(
    ...
    <style>
        .stApp {
            ...
            color: #111827 !important; /* hitam gelap */
        }
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["📊 Ringkasan", "🎯 Performa", "📈 Data", "🔗 Jaringan", "💡 Rekomendasi"]
)
...
with tab1:
    ...
    with col1:
        st.info(
            "**Tantangan**: H&M mengelola 7 juta transaksi pelanggan di seluruh 51.232 produk. "
            "Sistem rekomendasi hybrid menggabungkan pendekatan collaborative filtering "
            "(melihat kesamaan perilaku antar pelanggan) dan content-based "
            "(melihat kesamaan karakteristik produk) untuk memberikan rekomendasi yang akurat dan beragam."
        )
    with col2:
        st.success(
            "**Dampak Solusi**: Mencakup 4,60% dari total produk yang tersedia | "
            "Mampu menghasilkan 6 juta rekomendasi per hari | "
            "Peningkatan retensi pelanggan melalui pengalaman belanja yang dipersonalisasi"
        )
    ...
    st.success(
        "Model hybrid memberikan hasil terbaik dengan menyeimbangkan akurasi prediksi "
        "dan keberagaman rekomendasi produk."
    )
...
with tab2:
    ...
    st.write(
        "RMSE mengukur rata-rata kesalahan prediksi rating. Model dengan RMSE lebih rendah memiliki prediksi yang lebih akurat."
    )
    rmse_df = model_df[["Model", "RMSE"]].sort_values("RMSE").reset_index(drop=True)
    colors = ["#10B981" if m == "Hybrid" else "#3B82F6" for m in rmse_df["Model"]]
    fig1 = px.bar(
        rmse_df,
        y="Model",
        x="RMSE",
    ...
    st.write(
        "Coverage menunjukkan persentase produk yang dapat direkomendasikan oleh model. Coverage tinggi berarti model tidak hanya merekomendasikan produk populer saja."
    )
    cov_df = (
        model_df[["Model", "Coverage"]]
    ...
    st.caption(
        "✓ Hybrid model mencakup 4,60% dari semua produk = keseimbangan antara akurasi dan keberagaman"
    )

    st.subheader("Product Diversity - Semakin Tinggi Semakin Baik")
    st.write(
        "Menunjukkan berapa banyak produk berbeda yang direkomendasikan model kepada pelanggan. Jumlah lebih tinggi berarti keberagaman rekomendasi."
    )
    prod_df = (
        model_df[["Model", "Products"]]
    ...
    st.caption(
        "✓ Hybrid model merekomendasikan 4.854 produk unik = keberagaman yang baik dibanding ALS (1.598)"
    )

    st.subheader("Daily Recommendation Capacity - Semakin Tinggi Semakin Baik")
    st.write(
        "Kapasitas model untuk menghasilkan rekomendasi per hari. Volume tinggi menunjukkan skalabilitas model untuk bisnis besar."
    )
    rec_df = (
        model_df[["Model", "Recommendations"]]
    ...
    st.caption(
        "✓ Hybrid model menghasilkan 6 juta rekomendasi per hari = skalabel untuk operasi H&M yang besar"
    )
...
with tab3:
    ...
    c1.write(
        ...
        "Menunjukkan pelanggan dengan frekuensi pembelian tertinggi (power user). Insight: "
        "Pelanggan ini adalah high-value customer yang penting untuk retensi."
    )
    c2.write(
        f"**Produk Teratas**: {graph_stats['top_product']:,} pelanggan\n\n"
        "Menunjukkan produk yang dibeli oleh jumlah pelanggan terbanyak (populer). Insight: "
        "Produk ini adalah best-seller dan harus selalu tersedia."
    )
    ...
    st.write(
        ...
        "sementara beberapa pelanggan (power users) membeli sangat banyak. Insight: Sebagian besar pelanggan "
        "adalah casual buyers, ada peluang untuk meningkatkan frequency melalui rekomendasi."
    )
    np.random.seed(42)
    ...
    fig_dist = px.histogram(
        ...
        labels={"x": "Jumlah Produk", "y": "Jumlah Pelanggan"},
...
with tab4:
    ...
    st.info(
        "Jaringan ini menunjukkan hubungan antara pelanggan dan produk yang mereka beli. "
        "Struktur bipartite memungkinkan kami menemukan: (1) pelanggan serupa melalui produk yang mereka beli, "
        "(2) kluster produk yang sering dibeli bersama, (3) pelanggan atau produk yang paling berpengaruh dalam jaringan."
    )
    ...
    st.caption(
        "Jaringan sparse (kepadatan rendah) adalah tipikal untuk struktur e-commerce bipartite. "
        "Node = entitas (pelanggan/produk), Edge = hubungan pembelian"
    )
    ...
    # Posisi node: kiri = customer, kanan = produk
    cust_x = [-1] * n_cust
    ...
    for i in range(n_cust):
        ...
        targets = np.random.choice(n_prod, size=num_edges, replace=False)
        for j in targets:
    ...
    fig_net.add_trace(
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                size=cust_sizes, color="#EF4444", line=dict(width=1.5, color="#B91C1C")
            ),
            text=[f"C{i:02d}" for i in range(n_cust)],
            textposition="middle center",
            textfont=dict(size=9, color="#ffffff"),
            hovertemplate="<b>%{text}</b><br>Degree: %{marker.size:.0f}<extra></extra>",
            name="Customers",
            showlegend=True,
        )
    )
    ...
    fig_net.add_trace(
        go.Scatter(
            ...
            marker=dict(
                ...
                color="#0EA5E9",
                symbol="square",
                line=dict(width=1.5, color="#0369A1"),
            ),
            text=[f"P{i:02d}" for i in range(n_prod)],
            textposition="middle center",
    ...
    st.caption(
        "🔴 Merah = Pelanggan | 🔵 Biru = Produk | Ukuran Node = Jumlah koneksi (degree) | Garis Abu = Hubungan pembelian"
    )
    ...
    with col1:
        ...
        st.write(
            "- Node pelanggan besar = banyak produk dibeli\n"
            "- Strategi: VIP program, exclusive offers, dedicated support\n"
            "- Contoh: C000001 dengan 407 pembelian (power user)"
        )
        st.write("\n**📦 High-Degree Products (Popular Items)**")
        st.write(
            "- Node produk besar = dibeli banyak pelanggan\n"
            "- Strategi: Stock management, promotional bundling\n"
            "- Contoh: P866731 dibeli 108 pelanggan (best seller)"
        )
    with col2:
        st.write("**👥 Similar Customer Detection**")
        st.write(
            "- Pelanggan terhubung ke produk sama = similar taste\n"
            "- Metode: Collaborative filtering memanfaatkan pola ini\n"
            "- Aplikasi: 'Customers who bought X also bought Y'"
        )
        st.write("\n**🔗 Product Clustering & Co-purchase**")
        st.write(
            "- Produk sering dibeli bersama = 1 cluster\n"
...
with tab5:
    ...
    st.info(
        "Lihat rekomendasi produk untuk pelanggan contoh berdasarkan riwayat pembelian mereka dan model hybrid recommendation."
    )
    ...
    samples = {
        "C000001": {
            "purchases": [
                {
                    "product": "P866731",
                    "name": "Kaos Casual Biru",
                    "date": "2025-11-15",
                    "price": 49000,
                },
                {
                    "product": "P866732",
                    "name": "Celana Denim Premium",
                    "date": "2025-12-01",
                    "price": 89000,
                },
                {
    ...
    selected = st.selectbox(
        "Pilih pelanggan untuk melihat rekomendasinya:",
        list(samples.keys()),
        format_func=lambda x: f"{x} (Example)",
    )
    ...
    if selected:
        ...
        st.dataframe(
            hist_df[["product", "name", "date", "price"]],
            use_container_width=True,
            hide_index=True,
        )
        st.caption(
            f"Total: {len(data['purchases'])} produk | Total pembelian: Rp {sum([p['price'] for p in data['purchases']]):,}"
        )
...
st.markdown("---")
st.markdown(
    "<center style='color:#666; font-size:0.9em;'>H&M Recommendation System | Hybrid Collaborative + Content Analytics</center>",
    unsafe_allow_html=True,
)
