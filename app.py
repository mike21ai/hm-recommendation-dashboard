...
# Paksa seluruh app pakai background putih + font hitam
st.markdown(
    """
    <style>
        .stApp {
...
with tab3:
    ...
    fig_dist.update_layout(height=350, plot_bgcolor="#f9fafb", paper_bgcolor="#ffffff", font=dict(color="#111827"))
...
with tab4:
    ...
    st.info(
        "Jaringan ini menunjukkan hubungan antara pelanggan dan produk yang mereka beli. Struktur bipartite memungkinkan kami menemukan: (1) pelanggan serupa melalui produk yang mereka beli, (2) kluster produk yang sering dibeli bersama, (3) pelanggan atau produk yang paling berpengaruh dalam jaringan."
    )
    ...
    st.caption(
        "Jaringan sparse (kepadatan rendah) adalah tipikal untuk struktur e-commerce bipartite. Node = entitas (pelanggan/produk), Edge = hubungan pembelian"
    )
    ...
    fig_net.add_trace(
        go.Scatter(
            ...
            y=cust_y,
            mode="markers+text",
            marker=dict(size=cust_sizes, color="#EF4444", line=dict(width=1.5, color="#B91C1C")),
    ...
    fig_net.add_trace(
        go.Scatter(
            ...
            y=prod_y,
            mode="markers+text",
            marker=dict(size=prod_sizes, color="#0EA5E9", symbol="square", line=dict(width=1.5, color="#0369A1")),
...
with tab5:
    ...
    if selected:
        ...
        st.info(
            "💡 **Insight**: Model hybrid direkomendasikan karena memberikan keseimbangan terbaik antara akurasi dan keberagaman produk yang direkomendasikan."
        )
